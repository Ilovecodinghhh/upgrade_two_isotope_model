...
# -*- coding: utf-8 -*-
"""
Created on Wed Sep  3 12:41:32 2025
...
ONG_nodata = merged_df['ONG_Avg'][merged_df['dD_CH4_MEAN_x'].isna()].sum()
Coal_nodata = merged_df['Coal_Avg'][merged_df['dD_CH4_MEAN_y'].isna()].sum()
...
Total_emissions = pd.merge(coal_dD_All, Total_emissions, on='COUNTRY', how='left')
Total_emissions = pd.merge(ONG_dD_All_new, Total_emissions, on='COUNTRY', how='left')
Total_emissions['dD_CH4_MEAN_y'].fillna(dD_coal_glob, inplace=True)
Total_emissions.fillna(0, inplace=True)
Emissions_df = pd.DataFrame()
Country_FF_dD = pd.DataFrame()
for i in range(2, 55):
    ...
    Country_FF_dD[f'{1968+i}'] = (Total_emissions[column_name1]*Total_emissions[column_name3]
                                    + Total_emissions[column_name2]*Total_emissions['dD_CH4_MEAN_y']) / (Total_emissions[column_name1] + Total_emissions[column_name2])
    
# Add the 'Country' column to the result DataFrame
Emissions_df.insert(0, 'COUNTRY', Total_emissions['COUNTRY'])
Country_FF_dD.insert(0, 'COUNTRY', Total_emissions['COUNTRY'])

# Now calculate global average trend over time for dD
# First sum coal and fossil emissions for each year and then weight each country
...
# Calculate weighted average for each year for each country
dD_FF_AnnAvgA = Country_FF_weight.copy()  # Make a copy of df1 to store the result
dD_FF_AnnAvgA.iloc[:, 1:] = Country_FF_weight.iloc[:, 1:].values * Country_FF_dD.iloc[:, 1:].values
# Sum each column (excluding the first column)
sums = dD_FF_AnnAvgA.iloc[:, 1:].sum()

# Create a new DataFrame with the results
dD_FF_AnnAvg = pd.DataFrame({
    'Column': sums.index,
    'Sum': sums.values
}).reset_index(drop=True)
# For any time with NaN for a country, insert global average
for i in range(0, 53):
    Country_FF_dD[f'{1970+i}'].fillna(dD_FF_AnnAvg.iloc[i, 1], inplace=True)


#%% Monte Carlo analysis incoroporating uncertainty in source signatures

# First coal
coal_dD_AnnAvg_MC = pd.DataFrame()
R_coal_dD_AnnAvg_MC = pd.DataFrame()
for i in range(0, 1000):
    # First, create and reset dataframe of coal with production weights, mean coal sig and mean coal stdev
    Country_coal_std_merge = pd.merge(coal_dD_All, merged_df_std[['COUNTRY', 'Coal_StdErr']], on='COUNTRY')
    ...
    Country_coal_std_merge_weight['MC'] = Country_coal_std_merge_weight['dD_CH4_MEAN_y'] + (random_gaussians * Country_coal_std_merge_weight['Coal_StdErr'])
    ...
    coal_dD_AnnAvg_MC[f'{i}'] = Country_coal_std_merge_weightB.iloc[:, 3:-1].sum()
    # Calculate relative
    R_coal_dD_AnnAvg_MC[f'{i}'] = Country_coal_std_merge_weightB.iloc[:, 3:-1].sum() - Country_coal_std_merge_weightB.iloc[:, 31].sum() #Relative to 1998

# Calculate statistics
coal_dD_mean = coal_dD_AnnAvg_MC.mean(axis=1).to_numpy()
coal_dD_std = coal_dD_AnnAvg_MC.std(axis=1).to_numpy()
R_coal_dD_mean = R_coal_dD_AnnAvg_MC.mean(axis=1).to_numpy()
R_coal_dD_std = R_coal_dD_AnnAvg_MC.std(axis=1).to_numpy()


#%% Next ONG
# A potentially important point here is that there is no temporal variability in the dD stdev of temporally varying countries 

ONG_dD_AnnAvg_MC = pd.DataFrame()
R_ONG_dD_AnnAvg_MC = pd.DataFrame()
for i in range(0, 1000):
    # First, create and reset dataframe of coal with production weights, mean coal sig and mean coal stdev
    Country_ONG_std_merge = pd.merge(merged_df_std[['COUNTRY', 'ONG_StdErr']], ONG_dD_All_new, on='COUNTRY')
    ...
    ONG_dD_AnnAvg_MC[f'{i}'] = Country_ONG_std_merge_weightB.iloc[:, 55:108].sum()
    # Calculate relative
    R_ONG_dD_AnnAvg_MC[f'{i}'] = Country_ONG_std_merge_weightB.iloc[:, 55:108].sum() - Country_ONG_std_merge_weightB.iloc[:, 83].sum() #Relative to 1998

# Calculate statistics
ONG_dD_mean = ONG_dD_AnnAvg_MC.mean(axis=1).to_numpy()
ONG_dD_std = ONG_dD_AnnAvg_MC.std(axis=1).to_numpy()
R_ONG_dD_mean = R_ONG_dD_AnnAvg_MC.mean(axis=1).to_numpy()
R_ONG_dD_std = R_ONG_dD_AnnAvg_MC.std(axis=1).to_numpy()


#%% Now total fossil

FF_dD_AnnAvg_MC_EDGAR = pd.DataFrame()
R_FF_dD_AnnAvg_MC_EDGAR = pd.DataFrame()
for k in range(0, 1000):
    ...
    Country_FF_dD_std = pd.DataFrame()
    for i in range(2, 55):
        column_name1 = f'{1968+i}_x'
        column_name2 = f'{1968+i}_y'  # Column55 corresponds to Column2, Column56 to Column3, etc.
        column_name3 = f'{1968+i}'
        Emissions_df_std[f'{1968+i}'] = Total_emissions[column_name1] + Total_emissions[column_name2]
        Country_FF_dD_std[f'{1968+i}'] = (Total_emissions[column_name1] * (Total_emissions[column_name3] + (random_gaussian_ONG * Country_ONG_std_merge_weight['ONG_StdErr']))
                                      + Total_emissions[column_name2] * (Total_emissions['dD_CH4_MEAN_y'] + (random_gaussian_Coal * Country_coal_std_merge_weight['Coal_StdErr']))) / (Total_emissions[column_name1] + Total_emissions[column_name2])
    # Add the 'Country' column to the result DataFrame
    Emissions_df_std.insert(0, 'COUNTRY', Total_emissions['COUNTRY'])
    Country_FF_dD_std.insert(0, 'COUNTRY', Total_emissions['COUNTRY'])
    # Now calculate global average trend over time for dD
    # First sum coal and fossil emissions for each year and then weight each country
    ...
    # Calculate weighted average for each year for each country
    dD_FF_AnnAvgA_std = Country_FF_weight.copy()  # Make a copy of df1 to store the result
    dD_FF_AnnAvgA_std.iloc[:, 1:] = Country_FF_weight.iloc[:, 1:].values * Country_FF_dD_std.iloc[:, 1:].values
    # Sum each column (excluding the first column)
    sums_std = dD_FF_AnnAvgA_std.iloc[:, 1:].sum()
    # Create a new DataFrame with the results
    FF_dD_AnnAvg_MC_EDGAR[f'{k}'] = pd.DataFrame({'Sum': sums_std.values}).reset_index(drop=True)
    # Calculate relative
    R_FF_dD_AnnAvg_MC_EDGAR[f'{k}'] = pd.DataFrame({'Sum': sums_std.values}).reset_index(drop=True) - sums_std.iloc[28]
    
# Calculate statistics
FF_dD_mean = FF_dD_AnnAvg_MC_EDGAR.mean(axis=1).to_numpy()
FF_dD_std = FF_dD_AnnAvg_MC_EDGAR.std(axis=1).to_numpy()
R_FF_dD_mean = R_FF_dD_AnnAvg_MC_EDGAR.mean(axis=1).to_numpy()
R_FF_dD_std = R_FF_dD_AnnAvg_MC_EDGAR.std(axis=1).to_numpy()
...
plt.plot(years, FF_dD_mean, color='green')
plt.plot(years, FF_dD_mean + 2 * FF_dD_std, color='green', linestyle=':', linewidth=3)
plt.plot(years, FF_dD_mean - 2 * FF_dD_std, color='green', linestyle=':', linewidth=3, label=r'2$\sigma$ total uncertainty')
plt.plot(years, R_FF_dD_mean + FF_dD_mean[28], label='Global Mean Fossil', color='black', linewidth=4)
plt.plot(years, R_FF_dD_mean + 2 * R_FF_dD_std+ FF_dD_mean[28], color='black', linestyle=':', linewidth=3)
plt.plot(years, R_FF_dD_mean - 2 * R_FF_dD_std+ FF_dD_mean[28], color='black', linestyle=':', linewidth=3, label=r'2$\sigma$ relative uncertainty')
plt.ylabel(r"${\delta}\mathrm{D-CH_4}$ (‰)")
plt.legend(loc='lower right')
plt.title(r"Fossil Fuel ${\delta}\mathrm{D-CH_4}$ (‰)")
plt.xlim(1998, 2022)
plt.ylim(-197, -190)

# Save data
ONG_dD = np.column_stack((years, ONG_dD_mean, ONG_dD_std))
np.savetxt('Output/ONG_dD_GlobUnc.csv', ONG_dD, delimiter = ',', fmt = '%+.3f')
Coal_dD = np.column_stack((years, coal_dD_mean, coal_dD_std))
np.savetxt('Output/Coal_dD_GlobUnc.csv', Coal_dD, delimiter = ',', fmt = '%+.3f')
FF_dD = np.column_stack((years, FF_dD_mean, FF_dD_std))
np.savetxt('Output/FF_dD_GlobUnc.csv', FF_dD, delimiter = ',', fmt = '%+.3f')
R_FF_dD = np.column_stack((years, R_FF_dD_mean, R_FF_dD_std))
np.savetxt('Output/R_FF_dD_GlobUnc.csv', R_FF_dD, delimiter = ',', fmt = '%+.3f')
# Save MC analysis for total fossil
FF_dD_MC_EDGAR = np.column_stack((years, FF_dD_AnnAvg_MC_EDGAR))
np.savetxt('Output/FF_dD_GlobMC_EDGAR.csv', FF_dD_MC_EDGAR, delimiter = ',', fmt = '%+.3f')
...
# Load FF_dD_Uncertainty
start_year = 1998
FF_dD_Unc = pd.read_csv('Output/FFStdErr_dD_1x1.txt', header=None)
FF_dD_Unc = FF_dD_Unc.values
unique_values = np.unique(FF_dD_Unc)
# Load 1x1 FF_dD maps
directory = 'Output/'
compiled_FF_dD = np.zeros((24, 180, 360))
for i in range(24):
    filename = f'FF_dD_1x1_{start_year + i}.txt'
    file_path = os.path.join(directory, filename)
    FF_dD = np.loadtxt(file_path, delimiter=',')
    compiled_FF_dD[i, :, :] = FF_dD
compiled_FF_dD_flip = np.flip(compiled_FF_dD, axis=1)


#%% Now run MC error analysis using 1x1 dD map and uncertainties 

# Define number of runs
numMC = 1000
# Initialize an array to store the results
FF_dD_AnnAvg_MC = np.zeros((24, numMC))
rFF_dD_AnnAvg_MC = np.zeros((24, numMC))
...
for i in range(numMC):
    ...
    gaussian_array = np.vectorize(lambda x: gaussian_map[x])(FF_dD_Unc)
    MC_Unc = FF_dD_Unc * gaussian_array
    MC_Unc = np.tile(MC_Unc, (24, 1, 1))
    MC_Unc_flip = np.flip(MC_Unc, axis=1)
    FF_dD_rand = compiled_FF_dD_flip + MC_Unc_flip
    # Multiply source sig map by emission weighting
    FF_dD_weight = FF_dD_rand * FF_ann_weight
    FF_dD_glob = FF_dD_weight.sum(axis=(1, 2))
    # Store the result in the array
    FF_dD_AnnAvg_MC[:, i] = FF_dD_glob
    rFF_dD_AnnAvg_MC[:, i] = FF_dD_glob - FF_dD_glob[0]
    
# Calculate statistics
FF_dD_AnnAvg = FF_dD_AnnAvg_MC.mean(axis=1)
FF_dD_AnnAvg_std = FF_dD_AnnAvg_MC.std(axis=1)
rFF_dD_AnnAvg = rFF_dD_AnnAvg_MC.mean(axis=1)
rFF_dD_AnnAvg_std = rFF_dD_AnnAvg_MC.std(axis=1)

# Save MC analysis for edgar
FF_dD_MC_CTCH4 = np.column_stack((years[28:-1], FF_dD_AnnAvg_MC))
np.savetxt('Output/FF_dD_GlobMC_CTCH4.csv', FF_dD_MC_CTCH4, delimiter = ',', fmt = '%+.3f')
...
axs[0].plot(years, coal_dD_mean, label='Coal avg', color='blue')
axs[0].plot(years, coal_dD_mean[28] + R_coal_dD_mean + 2 * R_coal_dD_std, color='blue', linestyle='--')
axs[0].plot(years, coal_dD_mean[28] + R_coal_dD_mean - 2 * R_coal_dD_std, color='blue', linestyle='--')
axs[0].plot(years, coal_dD_mean + 2 * coal_dD_std, color='blue', linestyle=':')
axs[0].plot(years, coal_dD_mean - 2 * coal_dD_std, color='blue', linestyle=':')
axs[0].set_ylabel(r"${\delta}\mathrm{D-CH_4}$ (‰)")
axs[0].legend(loc='lower right')
axs[0].set_title('Coal Data')
# Plot DataFrame 2 in the second subplot
axs[1].plot(years, ONG_dD_mean, label='ONG avg', color='orange')
axs[1].plot(years, ONG_dD_mean[28] + R_ONG_dD_mean + 2*R_ONG_dD_std, color='orange', linestyle='--')
axs[1].plot(years, ONG_dD_mean[28] + R_ONG_dD_mean - 2*R_ONG_dD_std, color='orange', linestyle='--')
axs[1].plot(years, ONG_dD_mean + 2 * ONG_dD_std, color='orange', linestyle=':')
axs[1].plot(years, ONG_dD_mean - 2 * ONG_dD_std, color='orange', linestyle=':')
axs[1].set_ylabel(r"${\delta}\mathrm{D-CH_4}$ (‰)")
axs[1].legend(loc='upper right')
axs[1].set_title('ONG Data')
# Plot DataFrame 3 in the third subplot
axs[2].plot(years, FF_dD_mean, label='EDGAR', color='green')
axs[2].plot(years, FF_dD_mean + 2 * FF_dD_std, color='green', linestyle=':')
axs[2].plot(years, FF_dD_mean - 2 * FF_dD_std, color='green', linestyle=':')
axs[2].plot(years, FF_dD_mean[28] + R_FF_dD_mean + 2 * R_FF_dD_std, color='green', linestyle='--')
axs[2].plot(years, FF_dD_mean[28] + R_FF_dD_mean - 2 * R_FF_dD_std, color='green', linestyle='--')
axs[2].plot(years[28:-1], FF_dD_AnnAvg, label='CTCH4', color='black', linewidth=5)
axs[2].plot(years[28:-1], FF_dD_AnnAvg[0] + rFF_dD_AnnAvg + 2 * rFF_dD_AnnAvg_std, color='black', linestyle='--')
axs[2].plot(years[28:-1], FF_dD_AnnAvg[0] + rFF_dD_AnnAvg - 2 * rFF_dD_AnnAvg_std, color='black', linestyle='--')
axs[2].plot(years[28:-1], FF_dD_AnnAvg + 2 * FF_dD_AnnAvg_std, color='black', linestyle=':')
axs[2].plot(years[28:-1], FF_dD_AnnAvg - 2 * FF_dD_AnnAvg_std, color='black', linestyle=':')
axs[2].set_xlabel('Year')
axs[2].set_ylabel(r"${\delta}\mathrm{D}$-CH$_4$ (‰)")
