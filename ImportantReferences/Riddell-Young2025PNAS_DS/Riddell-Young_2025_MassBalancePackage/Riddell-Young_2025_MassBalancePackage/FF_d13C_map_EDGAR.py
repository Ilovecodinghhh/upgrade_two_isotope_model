...
# -*- coding: utf-8 -*-
"""
Created on Tue Sep 10 10:42:13 2024
...
merged_stdev['FF_d13C_stdev'].fillna(FF_StdDev_Mean, inplace=True)
merged_stdev['FF_StdErr'].fillna(FF_StdErr_Mean, inplace=True)
merged_stdev['d13C_CH4_N_x'].fillna(0, inplace=True)
merged_stdev['d13C_CH4_N_y'].fillna(0, inplace=True)
...
data_grid_FF = np.full((len(lat_grid), len(lon_grid)), d13C_ONG_glob)
data_grid_Coal = np.full((len(lat_grid), len(lon_grid)), d13C_coal_glob)
    ...
    for i_lon, lon in enumerate(lon_grid):
        ...
        for i, data_geometry in enumerate(data_geometries):
            if data_geometry and p.within(data_geometry):
                data_grid_FF[i_lat, i_lon]       = merged_FF['FF_d13C_mean'].iloc[i]
                data_grid_Coal[i_lat, i_lon]     = merged_Coal['d13C_CH4_MEAN_y'].iloc[i]
                ...
                data_grid_FF_StdDev[i_lat, i_lon]   = merged_stdev['FF_d13C_stdev'].iloc[i]
                data_grid_Coal_N[i_lat, i_lon]      = merged_stdev['d13C_CH4_N_y'].iloc[i]
                data_grid_ONG_N[i_lat, i_lon]       = merged_stdev['d13C_CH4_N_x'].iloc[i]

# export csvs
np.savetxt('Output/FF_d13C_1x1.txt', data_grid_FF, delimiter=',', fmt='%+.3f')
np.savetxt('Output/Coal_d13C_1x1.txt', data_grid_Coal, delimiter=',', fmt='%+.3f')
np.savetxt('Output/CoalStdErr_d13C_1x1.txt', data_grid_Coal_StdErr, delimiter=',', fmt='%+.3f')
np.savetxt('Output/ONGStdErr_d13C_1x1.txt', data_grid_ONG_StdErr, delimiter=',', fmt='%+.3f')
np.savetxt('Output/FFStdErr_d13C_1x1.txt', data_grid_FF_StdErr, delimiter=',', fmt='%+.3f')
np.savetxt('Output/CoalStdDev_d13C_1x1.txt', data_grid_Coal_StdDev, delimiter=',', fmt='%+.3f')
np.savetxt('Output/ONGStdDev_d13C_1x1.txt', data_grid_ONG_StdDev, delimiter=',', fmt='%+.3f')
np.savetxt('Output/FFStdDev_d13C_1x1.txt', data_grid_FF_StdDev, delimiter=',', fmt='%+.3f')
np.savetxt('Output/CoalN_d13C_1x1.txt', data_grid_Coal_N, delimiter=',', fmt='%+.3f')
np.savetxt('Output/ONGN_d13C_1x1.txt', data_grid_ONG_N, delimiter=',', fmt='%+.3f')


#%% save annual maps for ONG and total fossil
...
for idx, year in enumerate(years):
    ...
    # update US/China ONG d13C
    year_df.loc[year_df['COUNTRY'] == 'united states', 'd13C_CH4_MEAN_x'] = US_ONG_d13C.iat[idx]
    year_df.loc[year_df['COUNTRY'] == 'china', 'd13C_CH4_MEAN_x'] = China_ONG_d13C.iat[idx]
    year_df.loc[year_df['COUNTRY'] == 'canada', 'd13C_CH4_MEAN_x'] = Canada_ONG_d13C.iat[idx]

    # recalc FF d13C mean
    year_df['FF_d13C_mean'] = (
        year_df['ONG_Avg'] * year_df['d13C_CH4_MEAN_x'] +
        year_df['Coal_Avg'] * year_df['d13C_CH4_MEAN_y']
    ) / (year_df['ONG_Avg'] + year_df['Coal_Avg'])

    year_df['FF_d13C_mean'].fillna(d13C_FF_glob, inplace=True)
    year_df['d13C_CH4_MEAN_x'].fillna(d13C_ONG_glob, inplace=True)

    # [step 5] map countries to grids
    ff_values, ong_values = [], []
    for country in grid_with_country['SOVEREIGNT']:
        if pd.isna(country):
            ff_values.append(d13C_FF_glob)
            ong_values.append(d13C_ONG_glob)
        else:
            row_idx = country_to_row.get(country, None)
            if row_idx is not None:
                ff_values.append(year_df.at[row_idx, 'FF_d13C_mean'])
                ong_values.append(year_df.at[row_idx, 'd13C_CH4_MEAN_x'])
            else:
                ff_values.append(d13C_FF_glob)
                ong_values.append(d13C_ONG_glob)
    ...
    # [step 7] save
    np.savetxt(f"Output/FF_d13C_1x1_{year}.txt", grid_FF, delimiter=',', fmt='%+.3f')
    np.savetxt(f"Output/ONG_d13C_1x1_{year}.txt", grid_ONG, delimiter=',', fmt='%+.3f')
...
cbar = plt.colorbar(shrink=.5)
cbar.ax.tick_params(labelsize=8)
cbar.set_label('Coal ${\delta}^{13}C$-CH$_4$ (‰)', fontsize=8)
m.drawcoastlines(linewidth=.4, color='k')
m.drawcountries(linewidth=.4, color='k')
...
# Load data
ONG_d13C_1x1_1970 = pd.read_csv('Output/FF_d13C_1x1_1970.txt')
ONG_d13C_1x1_2021 = pd.read_csv('Output/FF_d13C_1x1_2021.txt')

# Define vmin and vmax for 13C
vmin13C = -60
vmax13C = -35

# Plot ONG d13C for 1970
data_gridONG_flip = np.flipud(ONG_d13C_1x1_1970)
m = Basemap(llcrnrlat=lower_left_lat, llcrnrlon=lower_left_lon,
              urcrnrlat=upper_right_lat, urcrnrlon=upper_right_lon)
plt.figure(dpi=1000)
...
cbar.ax.tick_params(labelsize=8)
cbar.set_label('ONG ${\delta}^{13}C$-CH$_4$ (‰)', fontsize=8)
m.drawcoastlines(linewidth=.4, color='k')
m.drawcountries(linewidth=.4, color='k')

# Plot ONG d13C for 2021
data_gridONG_flip = np.flipud(ONG_d13C_1x1_2021)
m = Basemap(llcrnrlat=lower_left_lat, llcrnrlon=lower_left_lon,
              urcrnrlat=upper_right_lat, urcrnrlon=upper_right_lon)
plt.figure(dpi=1000)
m.imshow(data_gridONG_flip, cmap='viridis', interpolation='nearest', norm=Normalize(vmin=vmin13C, vmax=vmax13C))
...
cbar = plt.colorbar(shrink=.5)  # Add a color bar
cbar.ax.tick_params(labelsize=8)
cbar.set_label('2021 ONG ${\delta}^{13}C$-CH$_4$ (‰)', fontsize=8)
m.drawcoastlines(linewidth=.4, color='k')
m.drawcountries(linewidth=.4, color='k')
