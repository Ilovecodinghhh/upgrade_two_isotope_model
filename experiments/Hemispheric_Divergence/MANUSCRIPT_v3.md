...
**Draft Date:** 2026-05-14 (v3 — Major Revision Round 2)

---

## Abstract

We show for the first time that post-2006 atmospheric methane (CH₄) growth is driven by asymmetric hemispheric microbial trends: Northern Hemisphere (NH) microbial emissions are increasing at +6.6 [+5.1, +8.1] Tg yr⁻² (2007–2020; 90% CI; 100% of Monte Carlo iterations positive), while Southern Hemisphere (SH) microbial emissions are stable (−1.1 [−2.4, +0.2] Tg yr⁻²). This hemispheric divergence — invisible to the one-box frameworks used by all prior isotope budget studies (Schwietzke et al., 2016; Riddell-Young et al., 2025; Fujita et al., 2025) and not resolved even in the two-box Bayesian approach of Dasgupta et al. (2025), who did not report separate hemispheric microbial trends — rules out globally symmetric mechanisms (e.g., uniform OH decline) and localizes the growth driver to NH tropical/boreal wetlands and livestock.

Our hemispheric two-box dual-isotope (δ¹³C + δD) mass-balance framework integrates observation-based interhemispheric CH₄ gradients, uncertainty-derived row scaling, and posterior predictive checks against EDGAR v7. We resolve three source categories — fossil fuel (FF), microbial (Mic), and biomass burning (BB) — in each hemisphere using 1000 Monte Carlo iterations.

Beyond the hemispheric microbial asymmetry, two further findings emerge. First, we identify a critical microbial δD source-signature uncertainty threshold of ~37‰ that determines whether δD improves or degrades methane source attribution. This threshold resolves a specific open contradiction in the literature: Riddell-Young et al. (2025) found "strong agreement" between δ¹³C and δD constraints (using σ ≈ 8‰, well below the threshold), while Thanwerdas et al. (2024) found δD "has only a minor influence" (using σ ≈ 128‰, far above). Below the threshold, δD reduces FF uncertainty by 38% in standard deviation (σ(FF): 31.0 → 19.2 Tg yr⁻¹) and by 53% in 90% confidence interval width (133 → 63 Tg yr⁻¹). Second, the OH-¹³C kinetic isotope effect (KIE) remains the sign-determining parameter for the post-2007 FF trend even in the dual-isotope hemispheric framework: Saueressig (1.0039) yields FF increasing (+2.9 Tg yr⁻²), Cantrell (1.0054) yields FF declining (−5.6 Tg yr⁻²). Adding δD breaks source-signature degeneracies, shifting the KIE's share of FF variance from 11% to 25% of a 38%-smaller total — exposing, rather than resolving, this irreducible uncertainty floor.
...
The hydrogen isotope system (δD-CH₄) has recently emerged as an independent constraint. Riddell-Young et al. (2025) presented the first harmonized global δD-CH₄ record (2005–2023) and found "strong agreement" between δ¹³C and δD mass balances, both indicating microbial-dominated growth. Dasgupta et al. (2025) employed a two-box Bayesian inversion with dual isotopes, finding that δD improves the biogenic/thermogenic partition. Fujita et al. (2025) went further, integrating ¹³C, D, and ¹⁴C in a particle filter and finding FF emissions 30% lower than previous isotope-based estimates. Yet Thanwerdas et al. (2024) found that δD "has only a minor influence" in their 3-D framework. No study has identified the quantitative conditions under which these contradictory assessments arise.

### 1.2 Scope and Contributions

This study addresses three interconnected questions:

1. **Does hemispheric resolution reveal spatial structure in methane growth that one-box models miss?** All prior isotope mass-balance studies — one-box (Schwietzke et al., 2016; Riddell-Young et al., 2025; Fujita et al., 2025) and even the two-box framework of Dasgupta et al. (2025) — have reported only global or hemispheric-total source trends. No study has tested whether NH and SH microbial trends diverge. We hypothesize that the post-2006 microbial increase is hemispheric in character, concentrated in the NH where the dominant growth mechanisms (tropical/boreal wetland expansion, livestock intensification, permafrost thaw) are located.
...
Our hemispheric two-box dual-isotope framework builds on Dasgupta et al. (2025) with three methodological advances: (i) observation-based interhemispheric CH₄ gradients from the NOAA Marine Boundary Layer reference (replacing prescribed ramps), (ii) uncertainty-derived weighting that gives δD equal influence (33% of the cost function), and (iii) systematic posterior predictive checks against EDGAR v7 that validate absolute emission levels and hemispheric partitioning. The primary novel contribution, however, is not methodological but scientific: the discovery that NH and SH microbial emissions have diverged since 2006, a finding that fundamentally changes the interpretation of the global methane budget.
...
### 3.1 Hemispheric Source Attribution: The NH/SH Microbial Divergence
...
**Northern Hemisphere.** NH microbial emissions are the dominant and most robust feature: +6.6 [+5.1, +8.1] Tg yr⁻² (100% of Monte Carlo iterations positive). NH FF emissions decline at −3.9 [−6.5, −1.5] Tg yr⁻² (significant at 90% CI). NH BB increases at +2.3 [+0.3, +4.0] Tg yr⁻² (98% positive); however, given that the BB absolute level is biased high (Section 3.2), this trend should also be interpreted with caution — it may partly reflect a solver artifact rather than a real fire trend.

**Southern Hemisphere.** SH microbial emissions are stable: −1.1 [−2.4, +0.2] Tg yr⁻² (not significant). SH FF shows a modest positive trend: +1.3 [+0.1, +2.8] Tg yr⁻² (95.6% positive). SH BB is negligible.
...
**The hemispheric microbial divergence is the central finding of this study.** NH microbial emissions are increasing at +6.6 Tg yr⁻² while SH microbial emissions are stable (−1.1 Tg yr⁻²) — a divergence of 7.7 Tg yr⁻² between hemispheres. This asymmetry is invisible to one-box models and has not been reported in prior two-box studies (Dasgupta et al., 2025, reported only total hemispheric emissions, not separate microbial trends). The divergence is robust across all 5 random seeds tested (σ(FF) spread < 1 Tg yr⁻¹; NH microbial trend consistently +6.1 to +6.8 Tg yr⁻²).

The NH concentration of microbial growth is consistent with multiple independent lines of evidence:
...
### 3.3 The δD Threshold: Resolving the Riddell-Young/Thanwerdas Contradiction

A specific open question in the field is whether δD-CH₄ measurements improve methane source attribution. Riddell-Young et al. (2025) found "strong agreement" between δ¹³C and δD constraints, while Thanwerdas et al. (2024) found δD "has only a minor influence" — a direct contradiction that has left the community without clear guidance on the value of expanding δD monitoring networks. We resolve this contradiction by identifying the quantitative condition that separates the two regimes.

We conducted a systematic sweep of microbial δD source-signature uncertainty from 0.5× to 16× baseline values (4–132‰, 1σ).

**[Figure 2.** The δD improvement curve. (a) FF 90% CI width as a function of microbial δD source-signature uncertainty (σ_Mic_δD), showing the crossover at ~37‰ where δD transitions from improving to degrading the inversion. Horizontal dashed line: δ¹³C-only reference CI (133 Tg yr⁻¹). (b) Percentage improvement relative to δ¹³C-only, with annotations for key literature uncertainty values. See `figures/fig_dD_threshold.pdf`.**]**

The key findings are:

1. At baseline uncertainty (σ_Mic_δD = 8.2‰), δD reduces the FF 90% CI width by **53%** (from 133 to 63 Tg yr⁻¹) and σ(FF) by **38%** (from 31.0 to 19.2 Tg yr⁻¹). Unless otherwise noted, we report the σ(FF) reduction (38%) as our primary metric and the CI-width reduction (53%) as a complementary measure.
2. The CI-width improvement remains positive (>10%) up to σ ≈ 34‰.
...
This threshold — the first quantitative identification of the regime boundary for δD's utility — directly resolves the literature contradiction: Riddell-Young et al. (2025) and Dasgupta et al. (2025) use relatively tight δD source-signature uncertainties (~8–15‰), placing them well below the threshold where δD helps. Thanwerdas et al. (2024) use uncertainties of ~128‰ in their 3-D framework — far above the threshold — explaining their finding that δD adds little. The two studies are not contradictory; they are sampling different regimes of the same underlying relationship.

The threshold is robust across all tested configurations: 3 KIE values, 3 lifetime scenarios, 5 data versions, and all year ranges (Section 6 of the δD threshold experiment). It is also stable across 1000-iteration bootstrap resampling (95% CI for the improvement at baseline: [48.5%, 53.5%]; probability of improvement > 0%: 100%).

The practical implication is clear: **δD's value for source attribution depends entirely on how well we know the hydrogen isotopic composition of microbial sources**. Investment in spatially resolved δD source-signature measurements (particularly for tropical wetlands, Arctic permafrost, and ruminant enteric fermentation) would have high scientific returns — provided the resulting community uncertainty remains below ~37‰.
...
### 3.6 Solver Diagnostics and Bound-Hit Analysis
...
**Any-bound hit rate and point-estimate vs. trend robustness.** Across all 18,400 solves (14 years × ~1,300 hemisphere-year combinations), 90% hit at least one bound constraint. This high rate is expected and does not compromise our conclusions, for the following reason. The 3-source formulation with 3 equations (mass balance, δ¹³C, δD) and non-negativity constraints is exactly determined at each time step — there is no overdetermination to absorb noise, so individual Monte Carlo draws frequently push one source fraction to its bound. In other words, point estimates at individual time steps are underdetermined in the presence of noise, and the bound-hitting reflects the solver regularizing an ill-posed point-wise problem.

However, our scientific conclusions rest on **trends across 14 years** (2007–2020), not on individual point estimates. The trend analysis aggregates information across time steps, and the Monte Carlo ensemble captures the full uncertainty propagation. The robustness of these trends is confirmed by multi-seed verification: across 5 independent random seeds, σ(FF) varies by only 1.0 Tg yr⁻¹ (5.2% of the mean) and the FF trend varies by 0.6 Tg yr⁻² — both negligible relative to the reported confidence intervals. The NH microbial trend (+6.6 Tg yr⁻²) is positive in 100% of iterations across all seeds, and the global FF declining trend is significant at 90% in all seeds. We therefore distinguish between point-estimate fragility (where bound-hitting is frequent and expected) and trend robustness (which is confirmed and seed-independent).

---

## 4. Discussion

### 4.1 What This Changes: From Global Averages to Hemispheric Mechanisms

Prior one-box isotope studies have consistently found that "microbial emissions drive post-2006 CH₄ growth" (Schwietzke et al., 2016; Basu et al., 2022; Riddell-Young et al., 2025; Fujita et al., 2025). Our hemispheric resolution transforms this statement from a global average into a mechanistic constraint. The finding that NH microbial emissions are increasing at +6.6 Tg yr⁻² while SH microbial emissions are stable changes the scientific interpretation in three specific ways:

**First, it eliminates globally symmetric explanations.** A uniform decline in OH sink strength — proposed by He et al. (2026b) as an alternative to microbial increases — would produce comparable microbial-like signals in both hemispheres. The observed asymmetry (+6.6 NH vs. −1.1 SH) is inconsistent with this mechanism acting alone. While hemispheric OH asymmetry is possible, it would need to be of a magnitude (~8% NH-only decline per decade) not supported by current observational constraints (Zhao et al., 2023).

**Second, it narrows the candidate source regions.** One-box studies cannot distinguish between, e.g., Amazonian wetlands (partly SH) and South Asian rice paddies (NH). The SH stability constrains the contribution of sources south of the equator — including southern Amazonia, southern African wetlands, and Australasian livestock — to be flat or declining over 2007–2020. The growth must originate north of the equator: boreal wetlands, NH tropical floodplains, South/Southeast Asian livestock, and/or Arctic permafrost thaw.

**Third, it changes the policy calculus.** If global microbial growth were spatially uniform, mitigation would require globally distributed action. The NH concentration of the signal implies that targeted interventions in specific NH regions — livestock management in South Asia, coal mine methane capture in China, and wetland monitoring in the boreal zone — would disproportionately slow global CH₄ growth.

No prior study — including Dasgupta et al. (2025), who employed a two-box framework — has reported this NH/SH microbial divergence. Dasgupta et al. focused on the biogenic/thermogenic partition and did not separately analyze hemispheric microbial trends. Our finding is therefore not an incremental methodological refinement but a new scientific result with direct implications for attribution and mitigation.
...
### 4.5 Implications for the Global Methane Pledge
...
3. **BB overestimate and trend caution.** Our BB estimate (54 Tg yr⁻¹) substantially exceeds GFED4s (~35 Tg yr⁻¹). The solver likely absorbs residuals from unresolved OH variability, transport error, or source-signature misspecification into BB — the least isotopically constrained category. Given this BB level bias, the NH BB trend (+2.3 Tg yr⁻²) should also be interpreted with caution; it may partly reflect a solver artifact rather than a genuine fire trend.

4. **No OH optimization.** We do not independently solve for OH trends, unlike He et al. (2026a,b) and Zhao et al. (2023). An unaccounted OH trend of ±1% decade⁻¹ could bias microbial trends by ~3–4 Tg yr⁻² and FF trends by ~2 Tg yr⁻² (Section 4.4).
...
1. **Post-2006 CH₄ growth is driven by a hemispheric divergence in microbial emissions — the first such finding from any isotope budget study.** NH microbial emissions are increasing at +6.6 [+5.1, +8.1] Tg yr⁻², while SH microbial emissions are stable (−1.1 [−2.4, +0.2] Tg yr⁻²). This asymmetry, invisible to one-box models and unreported even by the two-box study of Dasgupta et al. (2025), localizes the growth driver to NH tropical/boreal wetlands and livestock, rules out globally symmetric mechanisms (e.g., uniform OH decline), and changes the policy calculus from globally distributed mitigation to NH-targeted intervention.

2. **δD-CH₄ significantly improves source attribution — conditionally.** The improvement is a 38% reduction in σ(FF) (from 31.0 to 19.2 Tg yr⁻¹) and a 53% reduction in 90% CI width (from 133 to 63 Tg yr⁻¹), but only when microbial δD source-signature uncertainty is below a critical threshold of ~37‰. Above this threshold, δD degrades the inversion. This threshold — the first quantitative identification of the regime boundary for δD's utility — resolves the conflicting assessments of Riddell-Young et al. (2025; improvement with σ ≈ 8‰) and Thanwerdas et al. (2024; no improvement with σ ≈ 128‰), and establishes a clear target for observational investment.
