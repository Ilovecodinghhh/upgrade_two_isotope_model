...
The cause of accelerating atmospheric methane (CH₄) growth since 2006 remains contested, with isotope-based studies, satellite inversions, and bottom-up inventories reaching divergent conclusions. We present a hemispheric (NH/SH) two-box isotopic mass-balance framework that, for the first time, integrates both δ¹³C-CH₄ and δD-CH₄ with observation-based interhemispheric gradients and uncertainty-derived weighting to produce source attributions consistent with independent inventories. Our model resolves three source categories — fossil fuel (FF), microbial (Mic), and biomass burning (BB) — in each hemisphere using 1000 Monte Carlo iterations that propagate uncertainties in atmospheric observations, source signatures, kinetic isotope effects (KIE), lifetime, and interhemispheric transport.

Three principal findings emerge. First, Northern Hemisphere microbial emissions are increasing at +6.6 [+5.1, +8.1] Tg yr⁻² (2007–2020; 90% CI), while Southern Hemisphere microbial emissions are stable (−1.1 [−2.4, +0.2] Tg yr⁻²). This hemispheric asymmetry — invisible to one-box models — is consistent with enhanced emissions from tropical/boreal wetlands, livestock intensification, and warming-driven permafrost thaw concentrated in the NH. Second, global FF emissions are declining at −2.5 [−5.1, −0.05] Tg yr⁻² in both the two-box and one-box frameworks, and the model produces absolute FF levels (115 Tg yr⁻¹) and hemispheric partitioning (72% NH) consistent with EDGAR v7 — a first for isotope box-model studies. Third, systematic sensitivity experiments demonstrate that δD improves FF uncertainty by 53% relative to δ¹³C-only, but only when microbial δD source-signature uncertainty is below a critical threshold of ~37‰. The OH-¹³C KIE controversy (Saueressig 1.0039 vs. Cantrell 1.0054) still accounts for 25% of FF emission variance in the dual-isotope framework and determines the sign of the post-2007 FF trend.
...
We present a v4 hemispheric dual-isotope model that incorporates four methodological advances over previous box-model studies: (i) observation-based interhemispheric CH₄ gradients from the NOAA Marine Boundary Layer reference (replacing prescribed ramps), (ii) uncertainty-derived weighting that gives δD equal influence (33% of the cost function), (iii) systematic posterior predictive checks against EDGAR v7, and (iv) Fisher information analysis quantifying the information gain from hemispheric resolution.
...
Each model configuration runs 1000 iterations. Per iteration, we sample: (1) source signatures from their stated distributions, (2) KIE values from the distributions in Table 2, (3) atmospheric δ¹³C and δD with observational uncertainties, (4) the interhemispheric CH₄ gradient with ±10 ppb correlated + uncorrelated perturbation, and (5) the interhemispheric exchange timescale. The last year is trimmed from all trend analyses to avoid edge effects from the mass-balance finite-difference scheme.
...
**Figure 3** presents the δD improvement curve. The key findings are:
...
Our variance decomposition (following the methodology of Schwietzke et al., 2016) yields:
...
|--------|--------------|-----------------|
| OH-¹³C KIE | 39 | **25** |
| Source signatures | 48 | **48** |
| Lifetime | 7 | **14** |
| Residual/interaction | 6 | 13 |

Adding δD reduces KIE-driven variance from 39% to 25% — a meaningful but not transformative improvement. The KIE spread (|Cantrell − Saueressig|) on the FF trend shrinks from 8.6 to 5.7 Tg yr⁻² in the dual-isotope framework, a 34% reduction. However, the KIE remains the **second-largest** uncertainty component and determines the **sign** of the FF trend:
...
### 4.3 Implications for the Global Methane Pledge
...
3. **Prescribed BB emissions.** BB is solved alongside FF and Mic rather than prescribed from GFED4s. The increasing BB trend (+2.3 Tg yr⁻² in NH) warrants investigation against fire inventories.

4. **No OH optimization.** We do not independently solve for OH trends, unlike He et al. (2026a) and Zhao et al. (2023). OH changes could alias into source trends.
...
2. **δD-CH₄ significantly improves source attribution — conditionally.** The improvement is 53% in FF uncertainty reduction, but only when microbial δD source-signature uncertainty is below ~37‰. Above this threshold, δD degrades the inversion. This threshold explains the conflicting assessments of Riddell-Young et al. (2025; improvement) and Thanwerdas et al. (2024; no improvement), and establishes a clear target for observational investment.

3. **The OH-¹³C KIE remains an irreducible uncertainty floor** that determines the sign of the post-2007 FF trend even in a dual-isotope hemispheric framework. Source signatures account for the largest share of uncertainty (48%). Spatially resolved, time-varying source-signature measurements — not model complexity — are the binding constraint.
